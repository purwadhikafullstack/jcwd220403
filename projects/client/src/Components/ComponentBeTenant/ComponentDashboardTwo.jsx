...
const ComponentDashboardTwo = () => {
  ...
  const getDataRoom = async () => {
    try {
      const response = await axiosPrivate.get(
        `/getallpictureroom/${auth.tenantId}`
      );
  ...
  const getDataImagesRoom = async () => {
    try {
      const response = await axiosPrivate.get(`/roomimages/${roomId}`);
  ...
  const Open = async (room) => {
    const value = await axiosPrivate.get(`/roombyid/${room.id}`);
    setEditId(room.id);
  ...
  const createDataRooms = async () => {
    try {
      ...
      await axiosPrivate.post(`/room`, formData, config);
      setLoad(true);
      setTimeout(() => {
        setLoad(false);
        onClose(true);
  ...
  const deleteRooms = async (room) => {
    try {
      await axiosPrivate.delete(`/deleteroom/${room.id}`);
      getDataRoom();
      toast({
        title: 'Success',
                    ...
                    >
                      ...
                      <DrawerContent>
                                  ...
                                  <Stack>
                                    ...
                                    <CardFooter>
                                      <Button
                                        variant='outline'
                                        colorScheme='red'
                                        onClick={() => deleteRoomImages(image)}
                                      >
                                        Delete Picture
                                      </Button>
                                    </CardFooter>
                        ...
                        <DrawerFooter>
                          ...
                          <Button colorScheme='blue' onClick={createManyRooms}>
                            {load ? <Spinner /> : 'Save'}
                          </Button>
                        </DrawerFooter>
                      </DrawerContent>
