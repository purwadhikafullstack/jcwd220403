...
const DashboardOne = () => {
  ...
  const getDataForDashboard = async () => {
    try {
      const response = await axiosPrivate.get(`/property/${auth.tenantId}`);
  ...
  const editName = async (e) => {
    e.preventDefault();
    try {
      await axiosPrivate.patch(`/editname/${auth.tenantId}`, {
        name: nameProperty,
      });
      setLoad(true);
      setTimeout(() => {
  ...
  const editFacilities = async (e) => {
    ...
    try {
      const nameFacility = clickedItem
        ...
        .join(', ');
      await axiosPrivate.patch(`/editfacility/${auth.tenantId}`, {
        name: nameFacility,
      });
      setLoad(true);
      setTimeout(() => {
  ...
  const editDescription = async () => {
    try {
      await axiosPrivate.patch(`/editdescription/${auth.tenantId}`, {
        description: desc,
      });
      setLoad(true);
      setTimeout(() => {
  ...
  const editLocationDetail = async () => {
    try {
      await axiosPrivate.patch(`/editlocation/${auth.tenantId}`, {
        country,
        province,
        city,
      });
                ...
                <Carousel autoPlay infiniteLoop showArrows={true}>
                  {data.propertypictures.map((image, i) => (
                    <Box key={i}>
                      <Image
                        cursor='pointer'
                        style={{ filter: hover ? 'brightness(30%)' : 'none' }}
              ...
              <Text fontSize='md' color='gray.600' fontFamily='Helvetica'>
                ...
                >
                  ...
                  <DrawerContent>
                    <DrawerCloseButton />
                    <DrawerHeader>
                      Edit your fasility property <br />
                    </DrawerHeader>
                    <DrawerBody>
                      <Text color='red' textAlign='center' fontSize='12px'>
                        {msgNameFacility}
                      </Text>
                      <Flex
                        flexWrap='wrap'
                ...
                >
                  <PopoverTrigger>
                    <Button
                      ...
                      _hover='none'
                      marginLeft='10px'
                      onClick={() => setOpenPopoverDesc(true)}
                    >
                      <Icon
                        as={AiOutlineEdit}
                        cursor='pointer'
