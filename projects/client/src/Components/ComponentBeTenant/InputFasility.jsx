...
import {
    Box, Center, Text, Flex, Card,
    CardBody, Heading, useBreakpointValue, Button,
    Spinner
}
...
const InputFasility = () => {
    ...
    const handleSubmit = async () => {
        try {
            const DataFacilityString = clickedItem.map(facility => facility.title).join(", ")
            await axios.post("http://localhost:2000/api/betenant", {
        ...
        <Box>
            <Center>
                <Box width="650px">
                    ...
                    >
                        <Text fontFamily="Uniform Pro Medium" fontSize={fontSize} fontWeight="bold" textAlign={textAlign}>Fasilitas apa saja yang</Text>
                        <Text fontFamily="Uniform Pro Medium" fontSize={fontSize} fontWeight="bold" textAlign={textAlign}>ada di tempat Anda?</Text>
                        <Center>
                            {load ? (
                                <Button onClick={handleSubmit} backgroundColor="white" color="black" padding="15px" _hover={{ fontWeight: "bold" }} isDisabled="true">
                                    <Spinner color="black" />
                                </Button>
                            ) : (
                                <Button onClick={handleSubmit} backgroundColor="white" color="black" padding="15px"  _hover={{ fontWeight: "bold" }} 
