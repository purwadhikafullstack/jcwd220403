...
const searchlok = () => {
  const result = alllokasi.filter(name => name.city.toLowerCase().includes(lok.current.value.toLowerCase()))
  setFilterlok(result)
        ...
        >
          <Flex  justify="space-evenly" align="center" >
            ...
            <Flex justify="space-around" align="center" color="black" w={isMobile ? "70vw" : "30vw"} h="50px" borderRadius='full' cursor="pointer" border="1px" borderColor="gray.100" boxShadow="md" mr={isMobile ? 3: 12} onClick={onOpen}>
              ...
              <Text fontSize={isMobile ? "x-small" : "small"}>{lokasi ? new Date(state[0]["startDate"]).toLocaleString("en", {day : "numeric", month: "short", year: "numeric"}) + " - " + new Date(state[0]["endDate"]).toLocaleString("en", {day : "numeric", month: "short", year: "numeric"}) : "Minggu Manapun"}</Text>
          ...
          <DrawerBody>
            <Center>
              <Tabs isFitted variant="soft-rounded" colorScheme="orange" w={isMobile ? "90vw" : "60vw"}>
                <TabList mb='1em' bgColor="gray.100" borderRadius="2xl" overflow="hidden" h="12" >
                  <Tab display="block" alignSelf="center">
                    <Text fontSize="small">Lokasi</Text>
                    <Text fontSize={isMobile ? "xx-small" : "small"}>{lokasi ? lokasi : "Cari lokasi"}</Text>
                  </Tab>
                  <Tab display="block" alignSelf="center" >
                    <Text fontSize="small">Tanggal</Text>
                    <Text fontSize={isMobile ? "xx-small" : "small"}>{new Date(state[0]["startDate"]).toLocaleString("en", {day : "numeric", month: "short", year: "numeric"}) + " - " + new Date(state[0]["endDate"]).toLocaleString("en", {day : "numeric", month: "short", year: "numeric"})}</Text>
                  </Tab>
                ...
                <TabPanels>
                  <TabPanel>
                    <Center>
                      <Box>
                        <Input defaultValue={lokasi} w={isMobile ? '90vw' : '50vw'} ref={lok} onChange={searchlok} placeholder='Cari Kota' type="text" />
                        {filterlok ? 
                          <List>
                              {filterlok.map((item, index) => {
                                return (
                                  <ListItem p="2" value={item.city} key={index} _hover={{bgColor: "gray.100"}} cursor="pointer" onClick={() => {setLokasi(item.city); setFilterlok([])}} >{item.city}, {item.province}</ListItem>
                                  )
                                })}
                          </List>
                        : null}
